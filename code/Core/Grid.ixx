...
d1::Point DefaultGrid::toGrid(const d1::Point& p) const
{
    return { round(p.x), round(p.y) };
}


d1::int32 DefaultGrid::enlarge(d1::int32 d) const
{
    return round_up(d);
}


d1::Vector DefaultGrid::enlarge(const d1::Vector& v) const
{
    return { round_up(v.dx), round_up(v.dy) };
}


d1::Size DefaultGrid::enlarge(const d1::Size& s) const
{
    return { round_up(s.w), round_up(s.h) };
}


d1::nRect DefaultGrid::enlarge(const d1::nRect& r) const
{
    return d1::nRect::A{
        .left = round_down(r.l),
        .top = round_up(r.t),
